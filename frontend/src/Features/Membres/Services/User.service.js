...
export const find = async (data) => {
    const response = await fetch(urlBase + entityName + 'login/' + data.email + '/' + data.password);
    const { token } = await fetch(urlBase + entityName + 'login/' + data.email + '/' + data.password).then(res => res.json());
    console.log(token);
    if (response.ok) {
...
export const isAuth = async (token) => {
    const resp = await fetch(urlBase + entityName + 'auth/' + token, { method: "GET", headers: { "authorization": "bearers " + token }, body: JSON.stringify(token), })
    return (resp)
}
